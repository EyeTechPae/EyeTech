...
tracking_params = dict(winSize = (30,30), maxLevel = 2, criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03))
...
while rval:
    ...
    if len(tracking_paths) > 0:
        ...
        good_points = diff_feat_points < 1
        ...
        for tp in tracking_paths:
            #Calculate distance between feat points to keep the motion
            a=np.int32(tp)
            circle=a[1]
            b=a[1]-a[0]
            c=abs(b[0]+b[1])
            ...
            if (c >= 3):  
                ...
                cv2.circle(output_img,(circle[0], circle[1]) , 10, (0, 255, 0), -1)
                cv2.polylines(output_img, [np.int32(tp)], False, (0, 150, 0),2)
